...
- [Settings live in settings.json](.claude/memory/settings-live-in-json.md) — since 2026-09-21 driver settings are in `c:\Wise40\settings.json`, not the ASCOM Profile; registration still is, values survive a rebuild, and the root sub-key is `(root)` so PowerShell can parse the file

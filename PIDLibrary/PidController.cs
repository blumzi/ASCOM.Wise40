    ...
    public class PidController
    {
        ...
        public void SetOutputLimits(float minimum, float maximum)
        {
            ...
            if (_readOutput() < OutputMinimum)
                _writeOutput((ulong) OutputMinimum);
            else if (_readOutput() > OutputMaximum)
                _writeOutput((ulong) OutputMaximum);
        ...
        public virtual void Run()
        {
            ...
            Console.WriteLine("PID: Run()");
            IsRunning = true;
            _computeTimer.Change(TimeSpan.FromMilliseconds(0), SamplingRate);
        }

        public void Stop()
        {
            Console.WriteLine("PID: Stop()");
            _computeTimer.Change(Timeout.Infinite, Timeout.Infinite);
            IsRunning = false;
        }
        ...
        protected virtual void Compute()
        {
            ...
            _writeOutput((ulong) output);
            _lastInput = input;
            _lastError = error;
            Console.WriteLine("PID: Compute: output: {0}, _lastInput: {1}, _lastError: {2}", output, _lastInput, _lastError);
        }
